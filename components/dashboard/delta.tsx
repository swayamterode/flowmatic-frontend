...
/**
 * `inverse` marks metrics where a decrease is the good outcome (failed runs,
 * run duration, queue depth). Only the colour flips — the icon keeps pointing
 * in the direction the value actually moved.
 */
type DeltaPolarity = "normal" | "inverse";
      ...
      {variant === "badge" ? (
        <Badge
          className={cn(
            ...
            isFavorable ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500",
            className,
          )}
          data-slot="delta"
          variant="secondary"
        ...
        <div
          className={cn(
            ...
            value !== 0 && isFavorable ? "text-emerald-600 dark:text-emerald-400" : "",
            value !== 0 && !isFavorable ? "text-rose-600 dark:text-rose-400" : "",
            className,
          )}
          data-slot="delta"
          {...props}

...
type DecorPosition = NonNullable<VariantProps<typeof DecorIconVariants>["position"]>;

const decorCornersVariants = {
  all: ["top-left", "top-right", "bottom-right", "bottom-left"],
  diagonal: ["top-left", "bottom-right"],
} as const satisfies Record<string, readonly DecorPosition[]>;

type DecorCornersProps = {
  variant?: keyof typeof decorCornersVariants;
};

/** Renders the DecorIcon set for a card frame: all four corners, or just the top-left/bottom-right diagonal. */
export function DecorCorners({ variant = "all" }: DecorCornersProps) {
  return (
    <>
      {decorCornersVariants[variant].map((position) => (
